...
typedef struct {
  ...
  WhiskerMobileColor decoration_color; uint32_t alignment;
  float indent_logical_pixels, indent_percentage; uint64_t prepared_content;
} WhiskerMobileText;
typedef struct {
...
typedef struct {
  uint32_t command, kind, source, _reserved; uint64_t resource, generation;
  WhiskerStringRef identifier; WhiskerBytesRef data;
