...
func parseArgs(_ argv: [String]) -> Args? {
    ...
    guard let target, let output else { return nil }
    return Args(targetName: target, outputPath: output, inputs: inputs)
}
...
func render(targetName: String, elements: [ElementHit], modules: [ModuleHit]) -> String {
    ...
    var out = """
        ...
        // "Name")` applications in the `\(targetName)` SwiftPM target's
        // source set. Each module package owns its own copy of this
...
guard let args = parseArgs(CommandLine.arguments) else {
    FileHandle.standardError.write(Data(
        "usage: whisker-elements-codegen --target-name <name> --output <path> <input.swift>...\n".utf8
    ))
    exit(2)
}
