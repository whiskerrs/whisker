...
func render(targetName: String, crateName: String, elements: [ElementHit], modules: [ModuleHit]) -> String {
    ...
    var out = """
        ...
        //
        // Sourced from `@WhiskerElement("LocalTag")` and
        // `@WhiskerModule("Name")` applications in the `\(targetName)`
        // SwiftPM target's source set. Each `@WhiskerElement` is
...
guard let args = parseArgs(CommandLine.arguments) else {
    FileHandle.standardError.write(Data(
        "usage: whisker-elements-codegen --target-name <name> --crate-name <pkg> --output <path> <input.swift>...\n".utf8
    ))
    exit(2)
}
