...
impl CompiledTree {
    ...
    /// (`/detail/:id`), returning the matched route's id and the captured
    /// `:param` values. Returns `None` if no route matches.
    ///
    /// The string-routing counterpart of the typed [`Target::Id`]: a shared
    /// route's pattern matches once (every branch carries the same pattern),
    /// so the id + params it yields navigate to the right branch via the
    /// usual relative resolution. Segments are split on `/` (empty segments,
    /// e.g. the leading slash, ignored), a `:name` segment captures, and a
    /// literal segment must match verbatim.
    pub fn match_url(&self, url: &str) -> Option<(String, BTreeMap<String, String>)> {
        ...
        for info in self.by_path.values() {
            ...
            for (p, i) in pat.iter().zip(input.iter()) {
                if let Some(name) = p.strip_prefix(':') {
                    params.insert(name.to_string(), (*i).to_string());
                } else if p != i {
                    matched = false;
                    break;
                }
