...
| `whisker-macos` | Native Rust macOS Host and direct runtime composition boundary. It owns lifecycle/frame scheduling, viewport/scale sampling, cosmic-text intrinsic measurement with reusable prepared content, a transactionally retained Host projection, and Metal/wgpu painting for common and rounded boxes, rectangular overflow clips, and text. Input, accessibility, rounded descendant/path clips, exact non-solid borders, ellipsis/forced-direction text conformance, and complete group compositing remain incremental Host work. | generated `gen/macos` app |
