...
```
   ...
   whisker-protocol
   (Host-independent frame + typed intrinsic-measurement model, strict batch
    validation, and transactional reference validator; not yet wired into Lynx)
   ...
   (Host-independent retained scene + incremental frame journal + batched
    measurement state machine + Rust-facing Host trait; not yet wired into
    render! or a platform Host)
...
|---|---|---|
| `whisker` | Umbrella. Users `use whisker::prelude::*`; almost everything is a re-export surfaced through one import root. `SurfaceRuntime` is the first-class owner that accepts `render!` calls, forwards typed `css!` declarations into `whisker-style`, treats `raw_text` as Text content rather than a Host node, drives `SurfaceEngine`, and presents directly to a Host `FrameSink`. | user crates |
...
| `whisker-protocol` | Host-independent semantic frame and intrinsic-measurement types, stable IDs, strict batch validation, and transactional retained-tree validation. Built-in measurement payloads are typed for text, replaced content, native controls, and embedded surfaces; custom provider bytes carry an explicit schema version. Plain text is retained as a validated `SetText` presentation containing shaping inputs, resolved foreground paint, and an optional prepared Host object. It is currently a migration foundation and is not yet used by the production Lynx path. | future scene engine and renderer providers |
