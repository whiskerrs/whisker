...
- Defining artifact formats, commands, or project integration for embedding a
  Whisker application into an existing Host application. The runtime model may
  support embedding without deciding how embedded artifacts are built.
- Replacing the current function-call-style `render!` DSL with JSX, RSX, or an
  HTML-like syntax. New runtime internals must preserve the existing authoring
  model.
...
`#[whisker::main]` remains the single source-level entry point for a Whisker
application crate. It identifies a root factory and causes the crate to provide
a versioned Application interface and application descriptor. It is not the
Android, iOS, browser, or Desktop process entry point:
...
Conceptually, generated metadata contains:
...
A `RuntimeInstance` is one isolated execution of that descriptor: its module
instances, reactive owners and signals, scene, router, motion state, and scene
revisions are separate from other instances. Process-scoped infrastructure and
immutable code may be shared where their contracts permit it.

After constructing a registry for a `RuntimeInstance`, bootstrap resolves and
starts its Application module. Application startup therefore uses the same
dependency and lifecycle rules as other runtime components.

The primary standalone form and the optional embedded form use the same
application descriptor:

```text
Standalone
  generated Host process entry
    -> create one Host WhiskerView
    -> mount descriptor generated by #[whisker::main]

Embedded
  existing Host application
    -> create a Host WhiskerView where needed
    -> mount descriptor generated by #[whisker::main]
```

Standalone Whisker application development remains the primary product model.
Embedding does not introduce named `whisker::entry` functions or turn the
application into a collection of independently addressable roots. There is one
`#[whisker::main]` per application crate. If a Host links several independently
built embedded applications, generated package-qualified descriptors let it
select among them without exposing symbol names to Rust application code.

Host-specific input and output do not require parameters on `main`. An embedded
Host can provide ordinary versioned module interfaces before mount, and the
Whisker application can consume those interfaces like any other module.
...
11. `#[whisker::main]` remains the single application root declaration and
    provides one Application descriptor per Whisker application crate.
...
| `#[whisker::main]` | Provider and descriptor of the Application interface, resolved by standalone or embedded bootstrap |
