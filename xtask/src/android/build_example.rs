...
pub fn run(args: BuildExampleArgs) -> Result<()> {
    // The gradle project is now generated by `whisker-cng` (the CNG
    // pipeline `whisker run` triggers before invoking us). Standalone
    // `cargo xtask android build-example` therefore requires that the
    // user has run `whisker run --target android` at least once so
    // the gen tree exists. Long-term plan: the entire build-example
    // path moves into `whisker-cli` and this xtask command goes away;
    // for now the error message points at the workaround.
    ...
    if !example_dir.is_dir() {
        anyhow::bail!(
            "no generated Android project at {}\n\
             Run `whisker run --target android` (or `whisker build --target android`) \
             first to sync the gen/ tree.",
            example_dir.display()
        );
    }
