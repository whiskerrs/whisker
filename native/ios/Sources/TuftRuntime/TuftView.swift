...
public final class TuftView: LynxView {
    ...
    public override init(frame: CGRect) {
        ...
        guard let engine = tuft_bridge_engine_attach(viewPtr) else {
            NSLog("[TuftView] tuft_bridge_engine_attach returned NULL")
            return
        }
        self.engine = engine
    ...
    /// dirty. `userData` is the TuftView pointer set up in `init`.
    private static let requestFrameTrampoline:
        @convention(c) (UnsafeMutableRawPointer?) -> Void = { userData in
        guard let userData = userData else { return }
        let view = Unmanaged<TuftView>.fromOpaque(userData).takeUnretainedValue()
