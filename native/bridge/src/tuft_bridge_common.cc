// tuft_bridge_common.cc
//
// Platform-independent bridge implementation.
//
// All `tuft_bridge_*` C-ABI symbols defined here are safe to call from
// any host (iOS, Android, …). Per-platform plumbing (LynxView ivar
// access, event-system hooks) lives in tuft_bridge_ios.mm /
// tuft_bridge_android.cc.
...
#include "tuft_bridge.h"
#include "tuft_bridge_internal.h"
...
// TuftElement wraps a strong reference to a FiberElement. The C ABI hands
// out raw pointers; the wrapper keeps the underlying element alive via
// fml::RefPtr. tuft_bridge_release_element drops one strong reference.
struct TuftElement {
...
// `tuft_bridge_internal_dispatch_event` below; that looks up
...
// `tuft_bridge_engine_attach` is platform-specific (lives in
// tuft_bridge_ios.mm / tuft_bridge_android.cc); the common code only
// provides the `release` half.

extern "C" void tuft_bridge_engine_release(TuftEngine* engine) {
...
extern "C" bool tuft_bridge_dispatch(TuftEngine* engine,
                                     TuftTasmCallback callback,
                                     void* user_data) {
    if (engine == nullptr || engine->shell == nullptr || callback == nullptr) {
        return false;
    }
    TuftEngine* engine_capture = engine;
...
extern "C" TuftElement* tuft_bridge_create_element(TuftEngine* engine,
                                                   TuftElementTag tag) {
    if (engine == nullptr || engine->manager == nullptr) return nullptr;
    auto ref = CreateForTag(engine->manager, tag);
    if (!ref) return nullptr;
    return new TuftElement{std::move(ref)};
}

extern "C" void tuft_bridge_release_element(TuftElement* element) {
...
extern "C" void tuft_bridge_set_attribute(TuftElement* element,
                                          const char* key,
                                          const char* value) {
...
extern "C" void tuft_bridge_set_inline_styles(TuftElement* element,
                                              const char* css) {
    if (element == nullptr || !element->ref || css == nullptr) return;
    element->ref->SetRawInlineStyles(lynx::base::String(css));
}

extern "C" void tuft_bridge_append_child(TuftElement* parent,
                                         TuftElement* child) {
...
extern "C" void tuft_bridge_remove_child(TuftElement* parent,
                                         TuftElement* child) {
    if (parent == nullptr || child == nullptr || !parent->ref || !child->ref) {
        return;
    }
...
extern "C" void tuft_bridge_set_event_listener(TuftElement* element,
                                               const char* event_name,
                                               TuftEventCallback callback,
                                               void* user_data) {
...
extern "C" void tuft_bridge_set_root(TuftEngine* engine, TuftElement* page) {
...
extern "C" void tuft_bridge_flush(TuftEngine* engine) {
