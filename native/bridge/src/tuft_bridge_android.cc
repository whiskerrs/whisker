...
// Element PAPI work lives in tuft_bridge_common.cc.
//
// The whole file is gated on `__ANDROID__` so it compiles to nothing on
// non-Android platforms. Tuft's Cargo build scripts already select per
...
struct JvmHandles {
    JavaVM* jvm = nullptr;
    jclass tuft_view_class = nullptr;        // global ref
...
// Stored per-engine: the Kotlin TuftView weak global ref we call back into.
// Held in a side map keyed by engine pointer so the common code doesn't
// need to know about Java.
struct EngineJavaState {
    jobject tuft_view_weak = nullptr;  // weak global ref
};
...
std::map<TuftEngine*, EngineJavaState*>& JavaStateMap() {
    static std::map<TuftEngine*, EngineJavaState*> m;
    return m;
}
EngineJavaState* LookupJavaState(TuftEngine* engine) {
    auto& m = JavaStateMap();
...
lynx::shell::LynxShell* ExtractShell(JNIEnv* env, jobject lynx_view) {
    ...
    // GetObjectClass returns the runtime class (TuftView), but a JNI
...
// `user_data` is a `TuftEngine*` we keep paired with the Kotlin view in
// the JavaStateFor map.
extern "C" void RequestFrameTrampoline(void* user_data) {
    auto* engine = static_cast<TuftEngine*>(user_data);
    ...
    if (state == nullptr || state->tuft_view_weak == nullptr) return;
    ...
    jobject view = env->NewLocalRef(state->tuft_view_weak);
    if (view != nullptr) {
...
// JNI_OnLoad — cache the JVM pointer + TuftView class/method handles.
extern "C" JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void* /*reserved*/) {
    ...
    handles.request_frame_method = env->GetMethodID(
        handles.tuft_view_class, "requestFrameFromNative", "()V");
    env->DeleteLocalRef(local);
    if (handles.request_frame_method == nullptr) {
        LOGE("JNI_OnLoad: requestFrameFromNative not found on TuftView");
        return JNI_ERR;
    }
...
Java_dev_tuft_runtime_TuftView_nativeEngineAttach(
    ...
    TuftEngine* engine = tuft_bridge_internal_engine_create(shell);
    if (engine == nullptr) {
        LOGE("nativeEngineAttach: tuft_bridge_internal_engine_create failed");
        return 0;
    }
...
Java_dev_tuft_runtime_TuftView_nativeBindTuftView(
    ...
    state->tuft_view_weak = env->NewWeakGlobalRef(self);
    std::lock_guard<std::mutex> lock(JavaStateMutex());
...
Java_dev_tuft_runtime_TuftView_nativeEngineRelease(
    ...
    if (engine == nullptr) return;
    {
        ...
        if (state != nullptr) {
            if (state->tuft_view_weak != nullptr) {
                env->DeleteWeakGlobalRef(state->tuft_view_weak);
            }
            delete state;
            JavaStateMap().erase(engine);
        }
    }
    tuft_bridge_engine_release(engine);
}
...
// Rust runtime entry points. Defined by the user's `#[tuft::main]` crate
...
Java_dev_tuft_runtime_TuftView_nativeOnLynxEvent(
    ...
    bool handled = tuft_bridge_internal_dispatch_event(
