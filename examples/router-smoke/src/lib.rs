...
fn build_handle() -> RouterHandle {
    // The whole tree + registry, generated by `routes!`. The Switch is the
    // root; each tab is its own Stack sharing the `detail/:id` route. Each
    // route's id is its component name in snake_case (`Detail` → `detail`,
    // `ListScreen` → `list_screen`); `Detail` reads its `:id` via `use_param`.
