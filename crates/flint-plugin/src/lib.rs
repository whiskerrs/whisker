//! Plugin API for Flint CNG (Continuous Native Generation).
//!
//! A plugin is a Rust crate that exposes:
//! ```ignore
//! pub fn flint_plugin(ctx: &mut flint_plugin::PrebuildContext) {
//!     // Mutate ctx via typed mods (with_ios_info_plist, with_android_manifest, ...)
//! }
//! ```
//!
//! `flint prebuild` discovers plugins from `Cargo.toml` `[package.metadata.flint]`
//! and invokes `flint_plugin` on each, in dependency declaration order.
