...
//!
//! Text children:
//!   - String literals  → `.child(::tuft::build::raw_text(<lit>))`
//!   - `{expr}` blocks  → `.child(::tuft::build::raw_text(<expr>.to_string()))`
...
impl Node {
    fn to_tokens_stream(&self) -> TokenStream2 {
        match self {
            ...
            Node::Text(lit) => quote! {
                ::tuft::build::raw_text(#lit)
            },
            Node::Expr(expr) => quote! {
                ::tuft::build::raw_text((#expr).to_string())
            },
        }
    }
}
