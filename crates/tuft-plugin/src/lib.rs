//! Plugin API for Tuft CNG (Continuous Native Generation).
//!
//! A plugin is a Rust crate that exposes:
//! ```ignore
//! pub fn tuft_plugin(ctx: &mut tuft_plugin::PrebuildContext) {
//!     // Mutate ctx via typed mods (with_ios_info_plist, with_android_manifest, ...)
//! }
//! ```
//!
//! `tuft prebuild` discovers plugins from `Cargo.toml` `[package.metadata.tuft]`
//! and invokes `tuft_plugin` on each, in dependency declaration order.
