...
//! it is a follow-up — either through a tuft.rs config or by reading
...
impl Installer {
    ...
    fn host_skip(&self) -> Result<()> {
        eprintln!(
            "[tuft-dev-server] host target: install/launch is the user's job (run the binary yourself)"
        );
        Ok(())
    }
    ...
    fn ios_skip(&self) -> Result<()> {
        eprintln!(
            "[tuft-dev-server] iOS install/launch is not wired up in I4e — \
...
/// example's `AndroidManifest.xml` (or, eventually, a tuft.rs).
...
impl AndroidIdentity {
    pub(crate) fn for_package(package: &str) -> Self {
        // hello-world → rs.tuft.examples.helloworld /
        ...
        match package {
            "hello-world" => Self {
                application_id: "rs.tuft.examples.helloworld".into(),
                launcher: ".MainActivity".into(),
            },
            other => Self {
                application_id: format!("rs.tuft.examples.{}", other.replace('-', "")),
                launcher: ".MainActivity".into(),
            },
        }
    }
...
mod tests {
    ...
    fn android_identity_for_hello_world_matches_manifest() {
        ...
        assert_eq!(id.application_id, "rs.tuft.examples.helloworld");
    ...
    fn android_identity_for_unknown_package_strips_hyphens() {
        ...
        assert_eq!(id.application_id, "rs.tuft.examples.myshinyapp");
        assert_eq!(id.launcher, ".MainActivity");
    }
}
