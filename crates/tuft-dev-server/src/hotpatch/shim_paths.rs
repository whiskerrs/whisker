//! Resolve the on-disk paths of `tuft-rustc-shim` and
//! `tuft-linker-shim` for the dev session.
//!
//! Both shims live in the workspace's `tuft-cli` package, so they're
//! built into the cargo target dir alongside the main `tuft` binary.
...
//!   3. Otherwise spawn `cargo build -p tuft-cli --bin tuft-rustc-shim
//!      --bin tuft-linker-shim` from the workspace, then re-check.
...
pub fn expected_shim_paths(workspace_root: &Path) -> ShimPaths {
    ...
    ShimPaths {
        rustc_shim: bin("tuft-rustc-shim"),
        linker_shim: bin("tuft-linker-shim"),
    }
}
...
/// finds the right `tuft-cli` package via the workspace `members`
/// declaration.
pub fn resolve_shim_paths(workspace_root: &Path) -> Result<ShimPaths> {
    ...
    build_shims(workspace_root).context("build tuft-cli shim binaries")?;
    let paths = expected_shim_paths(workspace_root);
    anyhow::ensure!(
...
fn build_shims(workspace_root: &Path) -> Result<()> {
    eprintln!(
        "[tuft-dev-server] building shim binaries (`cargo build -p tuft-cli --bin tuft-rustc-shim --bin tuft-linker-shim`)…"
    );
...
mod tests {
    ...
    fn expected_paths_default_to_workspace_target_debug() {
        ...
        let linker_basename = p
            ...
            .and_then(|n| n.to_str())
            .unwrap();
        assert_eq!(rustc_basename, exe_name("tuft-rustc-shim"));
        assert_eq!(linker_basename, exe_name("tuft-linker-shim"));
        assert!(
    ...
    fn resolve_returns_existing_paths_without_rebuilding() {
        ...
        let rustc = target.join("debug").join(exe_name("tuft-rustc-shim"));
        let linker = target.join("debug").join(exe_name("tuft-linker-shim"));
    ...
    fn unique_tempdir() -> PathBuf {
        ...
        let p = std::env::temp_dir().join(format!("tuft-shim-paths-test-{pid}-{n}"));
