...
/// Mirrors `tuft_cli::linker_shim::CapturedLinkerInvocation` exactly.
...
pub struct LinkerCaptureConfig<'a> {
    /// Path to `tuft-linker-shim`. The shim is the value rustc sees
    /// for `-C linker=<…>`.
...
pub fn run_fat_build(
    ...
    if let Some(lc) = linker_capture {
        ...
        cmd.env("RUSTFLAGS", rustflags)
            .env("TUFT_LINKER_CACHE_DIR", lc.cache_dir)
            .env("TUFT_REAL_LINKER", lc.real_linker);
    }
...
/// Walk a `tuft-linker-shim` cache dir and collapse duplicates per
...
/// `target/.tuft/linker-args/`. Created on demand.
pub fn default_linker_cache_dir(workspace_root: &Path) -> PathBuf {
    workspace_root.join("target/.tuft/linker-args")
}
...
mod tests {
    ...
    #[test]
    fn default_linker_cache_dir_lives_under_target_dot_tuft() {
        let p = default_linker_cache_dir(Path::new("/tmp/ws"));
        assert!(p.ends_with("target/.tuft/linker-args"));
    }
