//! Plugin API for Whisker CNG (Continuous Native Generation).
//!
//! A plugin is a Rust crate that exposes:
//! ```ignore
//! pub fn whisker_plugin(ctx: &mut whisker_plugin::PrebuildContext) {
//!     // Mutate ctx via typed mods (with_ios_info_plist, with_android_manifest, ...)
//! }
//! ```
//!
//! `whisker prebuild` discovers plugins from `Cargo.toml` `[package.metadata.whisker]`
//! and invokes `whisker_plugin` on each, in dependency declaration order.
