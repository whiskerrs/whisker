...
use whisker_style::data_type_ext::{EasingFunction, Integer, Position, PositionKeyword, StepPosition};
use whisker_style::ext::*;
use whisker_style::shorthand::padding_margin::MarginValue;
use whisker_style::shorthand::{Animation, Background, BackgroundLayer, Border, Transition};
use whisker_style::value::{BorderRadius, FlexBasis, GridLine, GridTemplate, ImageRef, LineHeight, Repeated, Size};
...
fn animation_only_a_few_fields_set() {
    ...
    let s = Style::new().animation(Animation::new("x"));
    assert_eq!(s.to_string(), "animation: x;");
    // Now exercise the path with delay set but iteration_count omitted.
    let s = Style::new().animation(Animation::new("y").delay(50.ms()));
    assert!(s.to_string().contains("50ms"));
}

#[test]
fn transition_skip_some_fields() {
    let t = Transition::new(whisker_style::keyword::TransitionPropertyKind::All).delay(0.s());
    let s = Style::new().transition(t);
    assert!(s.to_string().contains("0s"));
}

#[test]
fn background_color_only_via_struct() {
    let s = Style::new().background(Background::new().color(Color::Named(NamedColor::Black)));
