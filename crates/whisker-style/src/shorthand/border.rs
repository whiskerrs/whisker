...
/// style. Use [`Style::border`] to apply to all four sides;
/// [`Style::border_top`], [`Style::border_right`],
/// [`Style::border_bottom`], [`Style::border_left`] target one side.
...
mod tests {
    ...
    #[test]
    fn border_full() {
        let s = Style::new().border(Border::new().width(px(1)).solid().color(Color::hex(0xCCCCCC)));
        assert_eq!(
            s.to_string(),
    ...
    fn border_partial_only_style() {
        let s = Style::new().border(Border::new().solid());
        assert_eq!(
            s.to_string(),
