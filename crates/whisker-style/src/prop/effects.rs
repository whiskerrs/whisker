...
mod tests {
    ...
    fn opacity_full_range() {
        assert_eq!(Style::new().opacity(0.0).to_string(), "opacity: 0;");
        assert_eq!(Style::new().opacity(0.5).to_string(), "opacity: 0.5;");
        assert_eq!(Style::new().opacity(1.0).to_string(), "opacity: 1;");
    }

    #[test]
    fn visibility_keyword() {
        assert_eq!(
            Style::new().visibility(Visibility::Hidden).to_string(),
            "visibility: hidden;"
        );
    }
    ...
    fn overflow_expands_to_both_axes() {
        let s = Style::new().overflow(Overflow::Hidden);
    ...
    #[test]
    fn box_shadow_inset() {
        let s = Style::new().box_shadow_inset(
            crate::data_type::Length::Zero,
            crate::data_type::Length::Zero,
            4.px(),
            crate::data_type::Length::Zero,
