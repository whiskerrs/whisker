...
mod tests {
    ...
    fn text_align_keywords() {
        let s = Style::new().text_align(TextAlign::Center);
    ...
    fn text_indent_value() {
        let s = Style::new().text_indent(px(20));
        assert_eq!(s.to_string(), "text-indent: 20px;");
    }

    #[test]
    fn vertical_align_keywords() {
        let s = Style::new().vertical_align(VerticalAlign::Middle);
    ...
    fn webkit_line_clamp_count() {
        let s = Style::new().webkit_line_clamp(2);
