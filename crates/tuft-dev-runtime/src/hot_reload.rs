                    ...
                    Ok(Envelope::Patch { mut table, lib_bytes_b64 }) => {
                        ...
                        match materialise_patch_dylib(&lib_bytes_b64) {
                            Ok(local) => {
                                ...
                                tuft_runtime::signal::wake_runtime();
                            }
                            Err(e) => {
...
///
/// On Android, `/data/data/<package>/cache/tuft-patches/` is the
...
fn patch_cache_dir() -> Option<std::path::PathBuf> {
    #[cfg(target_os = "android")]
    {
        ...
        if !pkg.is_empty() {
            return Some(
                std::path::PathBuf::from(format!("/data/data/{pkg}/cache/tuft-patches")),
            );
        }
        None
    }
    #[cfg(not(target_os = "android"))]
    {
        Some(std::env::temp_dir().join("tuft-patches"))
    }
}
