...
// happens in tuft_bridge_common.cc.
//
// Gated on `__APPLE__` for the same defense-in-depth reason as
// `tuft_bridge_android.cc`: any build system that scans the bridge
...
// per engine.
void InstallEventReporterIfNeeded(TuftEngine* engine, LynxView* view) {
    if (engine == nullptr ||
        tuft_bridge_internal_is_event_reporter_installed(engine)) {
        return;
    }
    ...
    tuft_bridge_internal_mark_event_reporter_installed(engine);
}

}  // namespace

extern "C" TuftEngine* tuft_bridge_engine_attach(void* lynx_view_ptr) {
...
extern "C" void tuft_bridge_log_hello(void) {
    NSLog(@"[TuftBridge] Hello from the Obj-C++ bridge");
}
