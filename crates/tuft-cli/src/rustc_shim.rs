//! `tuft-rustc-shim` binary's logic.
//!
...
//! ```
//!
//! when `RUSTC_WORKSPACE_WRAPPER=tuft-rustc-shim` is set. We do two
//! things, in order:
//!
//! 1. Dump the rustc invocation (full argv + crate name + timestamp)
//!    to JSON at `$TUFT_RUSTC_CACHE_DIR/<crate>-<microseconds>.json`.
...
//!
//! If `TUFT_RUSTC_CACHE_DIR` is unset, step 1 is skipped. That way a
//! stray `RUSTC_WORKSPACE_WRAPPER=tuft-rustc-shim` (left over from a
//! crashed `tuft run`) doesn't break ordinary `cargo build`.
...
/// Entry point called from `src/bin/tuft_rustc_shim.rs`.
pub fn run() -> Result<()> {
    let mut argv: Vec<String> = std::env::args().collect();
    if argv.len() < 2 {
        anyhow::bail!(
            "tuft-rustc-shim: expected `<wrapper> <rustc-path> [rustc-args...]`, \
             got {} arg(s)",
            argv.len(),
        );
    }
    ...
    if let Some(cache_dir) = std::env::var_os("TUFT_RUSTC_CACHE_DIR") {
...
mod tests {
    ...
    fn unique_tempdir() -> PathBuf {
        ...
        let p =
            std::env::temp_dir().join(format!("tuft-rustc-shim-test-{pid}-{n}"));
