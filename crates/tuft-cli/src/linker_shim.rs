...
//!
//! rustc, when told `-C linker=tuft-linker-shim`, invokes us as
//!
//! ```text
...
//! linker path from the `TUFT_REAL_LINKER` env var. The dev-server
...
//! configuration after a crashed `tuft run` doesn't silently break
...
/// Entry point called from `src/bin/tuft_linker_shim.rs`.
pub fn run() -> Result<()> {
    let mut argv: Vec<String> = std::env::args().collect();
    if argv.is_empty() {
        anyhow::bail!("tuft-linker-shim: empty argv");
    }
    let _shim_path = argv.remove(0);
    let linker_args = argv;

    if let Some(cache_dir) = std::env::var_os("TUFT_LINKER_CACHE_DIR") {
    ...
    let real_linker = std::env::var("TUFT_REAL_LINKER").context(
        "TUFT_REAL_LINKER not set; tuft-linker-shim has nothing to forward to. \
...
mod tests {
    ...
    fn unique_tempdir() -> PathBuf {
        ...
        let p = std::env::temp_dir().join(format!("tuft-linker-shim-test-{pid}-{n}"));
