...
pub struct Builder {
    ...
    /// builds the dev loop turns on `whisker/hot-reload`.
    features: Vec<String>,
...
mod tests {
    ...
    fn sample_capture() -> CaptureShims {
        CaptureShims {
            rustc_shim: PathBuf::from("/bin/whisker-rustc-shim"),
            linker_shim: PathBuf::from("/bin/whisker-linker-shim"),
    ...
    fn capture_env_vars_sets_both_shim_envs_and_cache_dirs() {
        ...
        let m = env_map(&c);
        assert_eq!(m["RUSTC_WORKSPACE_WRAPPER"], "/bin/whisker-rustc-shim");
        assert_eq!(m["WHISKER_RUSTC_CACHE_DIR"], "/cache/rustc");
        assert_eq!(m["WHISKER_LINKER_CACHE_DIR"], "/cache/linker");
        assert_eq!(m["WHISKER_REAL_LINKER"], "/usr/bin/cc");
    }
    ...
    fn capture_env_vars_includes_dash_c_linker_in_rustflags() {
        ...
        let m = env_map(&c);
        assert!(
            m["RUSTFLAGS"].contains("-Clinker=/bin/whisker-linker-shim"),
    ...
    fn capture_env_vars_preserves_existing_rustflags_when_set() {
        ...
        assert!(m["RUSTFLAGS"].contains("-Clinker=/bin/whisker-linker-shim"));
    }
