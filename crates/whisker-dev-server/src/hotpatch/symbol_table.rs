...
mod tests {
    ...
    /// `target/debug/whisker` always exists during a workspace-wide
    /// `cargo test` run because whisker-cli is a member of the workspace
    /// and the test harness builds every member's lib by default.
    /// We force its bin to exist by spawning `cargo build -p whisker-cli
    /// --bin whisker` once at the top of the test, but only if it isn't
