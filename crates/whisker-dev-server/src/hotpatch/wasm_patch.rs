// Derived from Dioxus packages/cli/src/build/patch.rs (MIT OR Apache-2.0).
// Whisker keeps the WebAssembly hot-patch transformer local so the Web build
// does not depend on the Dioxus CLI or its application-level build model.
// Keep the upstream native helpers intact so fixes can be synchronized as one
// transformer; Whisker currently exposes only the WebAssembly entry points.
#![allow(dead_code)]
...
pub fn create_wasm_jump_table(patch: &Path, cache: &HotpatchModuleCache) -> Result<JumpTable> {
    ...
    let element_kinds = new
        .elements
        .iter()
        .map(|element| match element.kind {
            ElementKind::Active { .. } => "active",
            ElementKind::Passive => "passive",
            ElementKind::Declared => "declared",
        })
        .collect::<Vec<_>>();
    let ifunc_table_initializer = new
        ...
        .with_context(|| format!("Missing ifunc table; element kinds: {element_kinds:?}"))?;
