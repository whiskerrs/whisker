...
impl BridgeRenderer {
    /// # Safety
    /// `engine` must point to a valid TuftEngine returned from
    /// `tuft_bridge_engine_attach`. Caller must ensure this renderer is
    /// only used inside a `tuft_bridge_dispatch` callback for the same
    /// engine.
    pub unsafe fn from_raw(engine: *mut TuftEngine) -> Option<Self> {
    ...
    fn engine_ptr(&self) -> *mut TuftEngine {
        self.engine.as_ptr()
    }
}

fn map_tag(tag: ElementTag) -> TuftElementTag {
    match tag {
        ElementTag::Page => TuftElementTag::Page,
        ElementTag::View => TuftElementTag::View,
        ElementTag::Text => TuftElementTag::Text,
        ElementTag::RawText => TuftElementTag::RawText,
        ElementTag::Image => TuftElementTag::Image,
        ElementTag::ScrollView => TuftElementTag::ScrollView,
    }
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct ElementHandle(*mut TuftElement);
...
impl Renderer for BridgeRenderer {
    ...
    fn create_element(&mut self, tag: ElementTag) -> Self::ElementHandle {
        let raw = unsafe { ffi::tuft_bridge_create_element(self.engine_ptr(), map_tag(tag)) };
        ElementHandle(raw)
    }

    fn release_element(&mut self, handle: Self::ElementHandle) {
        unsafe { ffi::tuft_bridge_release_element(handle.0) }
    }
    ...
    fn set_attribute(&mut self, handle: Self::ElementHandle, key: &str, value: &str) {
        ...
        unsafe { ffi::tuft_bridge_set_attribute(handle.0, key_c.as_ptr(), value_c.as_ptr()) }
    }
    ...
    fn set_inline_styles(&mut self, handle: Self::ElementHandle, css: &str) {
        ...
        unsafe { ffi::tuft_bridge_set_inline_styles(handle.0, css_c.as_ptr()) }
    }

    fn append_child(&mut self, parent: Self::ElementHandle, child: Self::ElementHandle) {
        unsafe { ffi::tuft_bridge_append_child(parent.0, child.0) }
    }

    fn remove_child(&mut self, parent: Self::ElementHandle, child: Self::ElementHandle) {
        unsafe { ffi::tuft_bridge_remove_child(parent.0, child.0) }
    }
    ...
    fn set_event_listener(
        ...
        unsafe {
            ffi::tuft_bridge_set_event_listener(
                handle.0,
                name_c.as_ptr(),
    ...
    fn set_root(&mut self, page: Self::ElementHandle) {
        unsafe { ffi::tuft_bridge_set_root(self.engine_ptr(), page.0) }
    }

    fn flush(&mut self) {
        unsafe { ffi::tuft_bridge_flush(self.engine_ptr()) }
    }
}
