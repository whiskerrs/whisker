...
///
/// The Host render loop pauses whenever a drive reports idle and resumes only
/// on a frame request. A `run_blocking` result arrives through the awaiting
/// task's waker, which must wake the Host even while vsync is parked.
///
/// ## How the harness models the worker→main-thread post
///
/// The worker wakes the stored task waker. The callback records a pending Host
/// drive, and the test thread consumes it before polling thread-local state.
mod cross_thread_wake {
    ...
    /// trampoline — there.
    static DRIVE_REQUESTED: AtomicBool = AtomicBool::new(false);

    extern "C" fn request_frame(_: *mut c_void) {
        DRIVE_REQUESTED.store(true, Ordering::Release);
    }

    fn install_host() {
        DRIVE_REQUESTED.store(false, Ordering::Release);
        crate::runtime_wake::set_request_frame_callback(Some(request_frame), std::ptr::null_mut());
    }
    ...
    fn drive_if_requested() -> bool {
        if DRIVE_REQUESTED.swap(false, Ordering::AcqRel) {
            tick();
            true
        } else {
            false
    ...
    /// "pauses" (stops ticking). Worker completions resume it only through the
    /// any-thread Host wake callback.
    fn drive_until(mut done: impl FnMut() -> bool) {
        ...
        while !done() && Instant::now() < deadline {
            if !vsync_idle {
                ...
                // Vsync paused. Service only a Host-posted drive request.
                drive_if_requested();
