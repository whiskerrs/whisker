...
{
    crate::diag::log("[tasks] spawn_local called");
    SPAWNER.with(|s| {
        s.borrow()
            .spawn_local(future)
...
pub fn run_until_stalled() {
    crate::diag::log("[tasks] run_until_stalled called — driving LocalPool");
    POOL.with(|p| {
    ...
    T: Send + 'static,
{
    ...
    crate::diag::log("[run_blocking] spawning worker thread");
    std::thread::spawn(move || {
        crate::diag::log("[run_blocking] worker thread started; running closure");
        let value = closure();
        crate::diag::log("[run_blocking] worker closure returned; hopping back to main thread");
        ...
        crate::main_thread::run_on_main_thread(move || {
            crate::diag::log("[run_blocking] on TASM thread: calling tx.send(value)");
