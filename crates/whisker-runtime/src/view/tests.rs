...
#[test]
fn suspense_renders_fallback_when_resource_is_loading() {
    use crate::reactive::{
        __reset_for_tests as reset_reactive, create_owner, dispose_owner, with_owner, Resource,
        ResourceState, RwSignal,
    };
    use crate::view::control_flow::suspense;
    use crate::view::set_attribute;

    // Build a Resource around an externally-owned signal so we can
    // flip Loading → Ready synchronously inside the test. The
    // public `resource()` helper spawns a worker thread we can't
    // pump from here without an active main-thread dispatcher.
    reset_reactive();
    ...
    with_installed_renderer(Box::new(renderer), || {
        with_owner(owner, || {
            let state: RwSignal<ResourceState<&'static str>> =
                RwSignal::new(ResourceState::Loading);
            let r: Resource<&'static str> = Resource::from_state(state);

            let _wrapper = suspense(
                r,
                || {
                    let h = create_element(ElementTag::Text);
                    set_attribute(h, "text", "loading");
                    View::Element(h)
                },
                |v: &'static str| {
                    let h = create_element(ElementTag::Text);
                    set_attribute(h, "text", v);
                    View::Element(h)
                },
            );

            // Initial render: should have produced a "loading" text.
            let texts_initial: Vec<String> = log
                .borrow()
                .iter()
                ...
                .collect();
            assert_eq!(texts_initial, vec!["loading".to_string()]);

            // Flip the resource to Ready — fallback should drop, the
            // children view should mount.
            state.set(ResourceState::Ready("done"));
            crate::reactive::flush();

            let texts_after: Vec<String> = log
                .borrow()
                .iter()
                .filter_map(|op| match op {
                    Op::SetAttr { key, value, .. } if key == "text" => Some(value.clone()),
                ...
                .collect();
            assert!(
                texts_after.contains(&"done".to_string()),
                "expected ready-state text `done`; got {texts_after:?}",
            );
        });
        dispose_owner(owner);
    });
