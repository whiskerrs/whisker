...
enum { WHISKER_MOBILE_ABI_MAJOR = 2, WHISKER_MOBILE_ABI_MINOR = 19 };
...
enum {
  ...
  WHISKER_OP_BACKDROP_BLUR, WHISKER_OP_IMAGE_RENDERING
};
enum {
  WHISKER_IMAGE_RENDERING_AUTO = 0, WHISKER_IMAGE_RENDERING_PIXELATED = 1,
  WHISKER_IMAGE_RENDERING_CRISP_EDGES = 2
};
enum {
...
enum {
  ...
  WHISKER_BACKGROUND_BOX_CONTENT = 2, WHISKER_BACKGROUND_BOX_BORDER_AREA = 3
};
...
typedef struct {
  uint32_t kind;
  uint8_t red, green, blue, _pad;
  float alpha;
  WhiskerStringRef name;
} WhiskerMobileColor;
...
typedef struct {
  uint32_t tag, flags;
  uint64_t node, parent, child;
  uint32_t index, member;
  int32_t integer;
  float scalar;
  uint64_t wide;
  const void* payload;
...
typedef struct {
  ...
  uint8_t mode, _pad[7];
  uint64_t surface;
  uint32_t scene_epoch, viewport_epoch;
  uint64_t frame_id, base_revision, target_revision;
  const WhiskerMobileOperation* operations;
...
typedef struct {
  uint32_t id;
  uint8_t value_kind, optional_kind, _pad[2];
  WhiskerStringRef name;
} WhiskerMobileMemberRegistration;
typedef struct {
  uint32_t element_type;
  uint8_t child_policy, measurement, _pad[2];
  WhiskerStringRef name;
...
typedef struct {
  uint16_t abi_major, abi_minor, protocol_major, protocol_minor;
  const WhiskerMobileElementRegistration* registrations;
  size_t registration_count;
} WhiskerMobileBootstrap;

typedef struct {
  uint64_t key, node;
  uint32_t element_type, kind;
  uint64_t environment_epoch;
  float known_width, known_height;
  uint32_t known_mask;
...
typedef struct {
  ...
  float width, height, first_baseline, last_baseline;
...
_Static_assert(sizeof(WhiskerMobileMeasureRequest) == 168, "WhiskerMobileMeasureRequest ABI drift");
...
typedef bool (*WhiskerMobileMeasureCallback)(void*, const WhiskerMobileMeasureRequest*, size_t,
                                             WhiskerMobileMeasureResponse*);
...
typedef void (*WhiskerMobileObserveModuleCallback)(void*, const uint8_t*, size_t,
