...
pub fn stage_module_swift_sources(
    ...
    let behaviours_path = sources_root.join("WhiskerModuleBehaviors.swift");
    std::fs::write(&behaviours_path, render_module_behaviours_swift(modules))
...
/// Render `WhiskerModuleBehaviors.swift`. Mirrors
/// `render_module_behaviours_kt` on the Android side — one
/// `LynxComponentRegistry.registerUI(...)` call per manifest
/// `[ios].behaviors` entry, all guarded by an `idempotent` flag so a
/// re-entrant `registerAll()` (multiple WhiskerView instances in
/// one process) doesn't scribble over the registry.
fn render_module_behaviours_swift(modules: &[crate::modules::ResolvedModule]) -> String {
    ...
    out.push_str(
        "// AUTO-GENERATED by whisker-build at gen-tree staging time.\n\
         ...
         // `[ios].behaviors` declarations. Calls Lynx's iOS\n\
         // `LynxComponentRegistry.registerUI(_:withName:)` runtime API\n\
         // once per registered tag, replacing the previous\n\
         // `LYNX_REGISTER_UI` Obj-C macro's `+load`-time registration.\n\
         //\n\
         // The Android sibling (`WhiskerModuleBehaviors.kt`) generates\n\
         // the same shape against `LynxEnv.inst().addBehavior(...)`.\n\n",
    );
    out.push_str("import Foundation\n");
    out.push_str("import Lynx\n\n");
    ...
    for m in modules {
        for b in &m.ios_behaviors {
            had_any = true;
            // Swift in a SwiftPM target registers classes with
            // Obj-C runtime under `<ModuleName>.<ClassName>` by
            // default. WhiskerModules is our SwiftPM target's
            // name (see `render_modules_package_swift`), so we
            // prefix the manifest's bare class name with it.
            //
            // The fallback to the bare name (without prefix)
            // handles modules whose author wrote an explicit
            // `@objc(ClassName)` on their Swift class — the
            // explicit `@objc(...)` overrides the default
            // module-prefixed runtime name, so `NSClassFromString`
            // resolves the bare name instead.
            out.push_str(&format!(
                "        do {{\n\
                 \u{20}           let cls = NSClassFromString({prefixed:?})\n\
                 \u{20}               ?? NSClassFromString({bare:?})\n\
                 \u{20}           if let cls = cls {{\n\
                 \u{20}               LynxComponentRegistry.registerUI(cls, withName: {tag_lit:?})\n\
                 \u{20}           }}\n\
                 \u{20}       }}\n",
                prefixed = format!("WhiskerModules.{}", b.class),
                bare = b.class,
                tag_lit = b.tag,
            ));
        }
    ...
    if !had_any {
        // Make the empty case explicit — keeps the body well-formed
        // and gives the compiler a stable shape to verify against.
        out.push_str("        // (no module declared `[ios].behaviors`)\n");
    }
