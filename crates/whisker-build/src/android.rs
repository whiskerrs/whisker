...
pub fn stage_module_kotlin_sources(
    ...
    // Generate `WhiskerModuleBehaviors.kt` — the bridge between
    // `whisker.module.toml`'s `[android].behaviors` declarations and
    // Lynx Android's runtime registry. `WhiskerView.kt` invokes
    // `WhiskerModuleBehaviors.registerAll()` once at init so every
    // declared `(tag, class)` is reachable from
    // `lynx_create_fiber_element_by_name`.
    //
    // We write the file unconditionally (even when no module declares
    // behaviours) so `WhiskerView.kt`'s static reference always
    // resolves. The empty-list case generates a no-op `registerAll`
    // body.
    std::fs::create_dir_all(&root).with_context(|| format!("mkdir -p {}", root.display()))?;
    let behaviours_path = root.join("WhiskerModuleBehaviors.kt");
    let body = render_module_behaviours_kt(modules);
    std::fs::write(&behaviours_path, body)
        .with_context(|| format!("write {}", behaviours_path.display()))?;
...
/// Render the auto-generated `WhiskerModuleBehaviors.kt` body. Each
/// declared `(tag, class)` produces a `LynxEnv.inst().addBehavior(...)`
/// call inside `registerAll()`. The single `RegisteredOnce` guard
/// flips on first call so a re-entrant `WhiskerView.init` (e.g.
/// multiple WhiskerViews in one process) doesn't double-register —
/// `addBehavior` is idempotent on tag but extra calls would scribble
/// over the registry slot.
fn render_module_behaviours_kt(modules: &[crate::modules::ResolvedModule]) -> String {
    let mut out = String::new();
    out.push_str(
        "// AUTO-GENERATED by whisker-build at gen-tree staging time.\n\
         // DO NOT EDIT — re-run `whisker run` / `whisker build` to refresh.\n\
         //\n\
         // Sourced from each dep crate's `whisker.module.toml`\n\
         // `[android].behaviors` declarations. The matching iOS path\n\
         // doesn't need this file because Obj-C++'s `LYNX_REGISTER_UI`\n\
         // macro generates a `+load`-time constructor that registers\n\
         // each tag against Lynx's behaviour registry at dylib load.\n\
         //\n\
         // When the `@WhiskerElement` KSP processor lands\n\
         // (planned follow-up to Phase 7-Φ.C), this file goes away\n\
         // and the registrations are produced by the KSP-generated\n\
         // sources directly.\n\n",
    );
    out.push_str("package rs.whisker.runtime.generated\n\n");
    out.push_str("import com.lynx.tasm.LynxEnv\n");
    out.push_str("import com.lynx.tasm.behavior.Behavior\n");
    out.push_str("import com.lynx.tasm.behavior.LynxContext\n");
    out.push_str("import com.lynx.tasm.behavior.ui.LynxUI\n");
    out.push_str("import java.util.concurrent.atomic.AtomicBoolean\n\n");
    out.push_str("object WhiskerModuleBehaviors {\n");
    out.push_str("    private val registered = AtomicBoolean(false)\n\n");
    out.push_str("    @JvmStatic\n");
    out.push_str("    fun registerAll() {\n");
    out.push_str("        if (!registered.compareAndSet(false, true)) return\n");
    out.push_str("        val env = LynxEnv.inst()\n");

    for m in modules {
        for b in &m.android_behaviors {
            // Anonymous Behavior subclass. `createUIFiber` is the
            // fiber-arch entry point — the only one Whisker actually
            // exercises since the bridge runs in fiber mode.
            // `createUI` (template-arch) falls through to the same
            // factory so generated code works in both modes.
            out.push_str(&format!(
                "        env.addBehavior(object : Behavior(\"{tag}\") {{\n\
                 \u{20}           override fun createUI(context: LynxContext): LynxUI<*> =\n\
                 \u{20}               {cls}(context)\n\
                 \u{20}           override fun createUIFiber(context: LynxContext): LynxUI<*> =\n\
                 \u{20}               {cls}(context)\n\
                 \u{20}       }})\n",
                tag = b.tag,
                cls = b.class,
