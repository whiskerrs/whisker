//! Per-crate Android compilation logic. Called from `lyra_build::compile()`
...
pub fn compile() -> Result<()> {
    ...
    let abi_dir = match abi.as_str() {
        "aarch64" => "arm64-v8a",
        other => anyhow::bail!(
            "lyra-build currently supports only arm64-v8a on Android (got {other})"
        ),
    };
    ...
    build
        ...
        .file(bridge_src.join("lyra_bridge_android.cc"))
